...
if __name__ == '__main__':
    ...
    best_model.save(f"{args.model_save_folder}/{len(best_model.layers)}_regr_model_" + str(datetime.datetime.now().date()) + ".h5",
                    save_format="h5")
    ...
    for j, fold_no in enumerate(range(0, 100, num_folds)):
        ...
        for k, (tr, valid) in enumerate(kfold.split(train_images, train_labels)):
            ...
            mse_per_fold_tr.append(scores[1])
            mae_per_fold_tr.append(scores[0])
            ...
            mse_per_fold_vd.append(scores[1])
            mae_per_fold_vd.append(scores[0])
    ...
    print("Means: ")
    print(np.mean(mse_per_fold_tr), np.mean(mae_per_fold_vd))

    # сохраним лучшую (минимаьную по средней квадратической ошибке) модель
    best_model_id = np.argmin(mse_per_fold_vd)
